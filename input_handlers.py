...
"""
 ...
 If an action is returned it will be attempted and if it's alid then
...
class BaseEventHandler(tcod.event.EventDispatch[ActionOrHandler]):
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        """Handle an event and return the next active eent handler."""
...
class PopupMessage(BaseEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        """Render the parent and dim the result, then print the mssage on top."""
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        key = event.sym
        if key in MOVE_KEYS:
            modifier = 1  # Holding modifer keys will speed up key movement.
...
class AreaRangedAttackHandler(SelectIndexHandler):
    ...
    Any area within the area will be affected.
    """
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        # so the player can see the affected area.
        console.draw_frame(
            x=x - self.radius - 1,
            y=y - self.radius - 1,
            width=self.radius**2,
...
class HistoryViewer(EventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        super().on_render(console)  # Draw the main stage as the background
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[MainGameEventHandler]:
        ...
        if event.sym in CURSOR_Y_KEYS:
            ...
            self.cursor = self.log_length - 1  # Move directory to the last message.
