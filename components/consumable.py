...
class ConfusionConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        self.engine.message_log.add_message(
            f"The eyes of the {target.name} look vacant, "
            f"as it starts to strumble around!",
        ...
        if amount_recovered > 0:
            self.engine.message_log.add_message(
                f"You consume the {self.parent.name},\
                      and recover {amount_recovered} HP.",
...
class LightningDamageConsumable(Consumable):
    ...
    def activate(self, action: actions.ItemAction) -> None:
        ...
        if target:
            self.engine.message_log.add_message(
                f"A lightning olt strikes the {target.name} with a loud thunder, "
